...
def load_prediction_data():
    ...
    pred_file = os.path.join(pred_dir, 'ev_charging_prediction_data.xlsx')
...
def load_actual_data():
    ...
    actual_file = os.path.join(actual_dir, '按日累计-台区R1-充电负荷数据.csv')
    ...
    # 转换日期列（第一列可能有特殊字符）
    date_col = df_actual.columns[0]
    df_actual['日期'] = pd.to_datetime(df_actual[date_col])
...
def extract_day_data_from_prediction(df, target_date):
    ...
    # 提取实际值和预测值
    actual_values = day_data['EV_Actual'].values
    pred_values = day_data['EV_Predicted'].values
...
def main():
    ...
    try:
        ...
        if len(results) > 0:
            ...
            # 保存为Excel
            output_file = os.path.join(output_dir, 'EV充电负荷预测评估指标.xlsx')
            ...
            # 保存为CSV（备份）
            output_csv = os.path.join(output_dir, 'EV充电负荷预测评估指标.csv')
